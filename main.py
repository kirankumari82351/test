...
from topic_utils import (
    build_upload_captions,
    forum_topic_short_name,
...
async def is_forum_chat(bot: Client, chat_id: int) -> bool:
    try:
...
async def ensure_topic_thread(bot: Client, chat_id: int, topic_name: str, batch_name: str):
    try:
        topic_name = sanitize_topic_name(topic_name)
        saved_thread_id = db.get_forum_topic(chat_id, topic_name)
        if saved_thread_id:
            return saved_thread_id

        created_topic = await bot.create_forum_topic(int(chat_id), topic_name)
        thread_id = (
            getattr(created_topic, "message_thread_id", None)
            or getattr(created_topic, "id", None)
        )
        if not thread_id:
            return None

        db.save_forum_topic(chat_id, topic_name, thread_id)
        await bot.send_message(
            int(chat_id),
            (
                f"<blockquote><b>🏷️ Topic:</b> {topic_name}\n"
                f"<b>🎯 Batch:</b> {batch_name}</blockquote>\n\n"
                "⚡ <b>All related videos and files will appear here!</b>"
            ),
            message_thread_id=thread_id,
        )
        return thread_id
    except Exception as exc:
        logging.error(f"Failed to create or reuse forum topic '{topic_name}': {exc}")
        return None
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:    
        ...
        links = parse_upload_entries(content)
        for entry in links:
            current_url = entry["url"].lower()
            if ".pdf" in current_url:
                pdf_count += 1
            elif current_url.endswith((".png", ".jpeg", ".jpg")):
                img_count += 1
            elif "v2" in current_url:
                v2_count += 1
            elif "mpd" in current_url:
                mpd_count += 1
            elif "m3u8" in current_url:
                m3u8_count += 1
            elif "drm" in current_url:
                drm_count += 1
            elif "youtu" in current_url:
                yt_count += 1
        ...
        if not links:
            await editable.edit("**❌ No valid URLs found in the txt file.**")
            os.remove(x)
            return
    ...
    try:
        ...
        await editable.edit("**❌ Invalid channel/group ID. Please send a numeric Telegram ID.**")
        return

    await editable.edit(
        "**Topic wise upload karna hai? (Natking-style: txt se `[Topic]` detect, forum threads, captions)**\n\n"
        "Send `yes` — topic-wise + forum (agar group me topics on hon)\n"
        "Send `no` ya `/d` — normal upload"
    )
    try:
        input8: Message = await bot.listen(editable.chat.id, timeout=timeout_duration)
        raw_text8 = (input8.text or "").strip().lower()
        await input8.delete(True)
    except asyncio.TimeoutError:
        raw_text8 = "/d"

    topic_wise_upload = raw_text8 in {"y", "yes", "ha", "haan", "hmm", "1", "true", "on"}
    forum_enabled = topic_wise_upload and await is_forum_chat(bot, channel_id)
    if topic_wise_upload and not forum_enabled:
        await bot.send_message(
            m.chat.id,
            "**Topic-wise caption enabled. Target chat forum-enabled nahi hai, isliye files normal chat me jayengi but detected topic caption me add hoga.**"
        )
    await editable.delete()
    ...
    count =int(raw_text)    
    arg = int(raw_text)
    announced_topics = set()
    try:
        for i in range(arg-1, len(links)):
            current_item = links[i]
            url = current_item["url"].replace("file/d/","uc?export=download&id=").replace("www.youtube-nocookie.com/embed", "youtu.be").replace("?modestbranding=1", "").replace("/view?usp=sharing","")
            link0 = current_item["url"]
            raw_topic = sanitize_topic_name(current_item.get("topic") or "General")
            detected_topic = forum_topic_short_name(raw_topic) if topic_wise_upload else raw_topic
            topic_thread_id = None
            if topic_wise_upload and forum_enabled:
                topic_thread_id = await ensure_topic_thread(bot, channel_id, detected_topic, b_name)
                if topic_thread_id and detected_topic not in announced_topics:
                    announced_topics.add(detected_topic)

            name1 = current_item["title"].replace("(", "[").replace(")", "]").replace("_", "").replace("\t", "").replace(":", "").replace("/", "").replace("+", "").replace("#", "").replace("|", "").replace("@", "").replace("*", "").replace(".", "").replace("https", "").replace("http", "").strip()
            if "," in raw_text3:
            ...
            try:
                cc, cc1, ccimg, cczip, ccm, cchtml = build_upload_captions(
                    count,
                    name1,
                    b_name,
                    CR,
                    topic_wise_upload,
                    detected_topic if topic_wise_upload else raw_topic,
                )
                ...
                if "drive" in url:
                    ...
                    try:
                        ...
                        time.sleep(1)
                        await bot.send_document(chat_id=channel_id, document=f"{name}.html", caption=cchtml, message_thread_id=topic_thread_id)
                    ...
                    try:
                        ...
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
                        await bot.send_document(chat_id=channel_id, document=f'{name}.{ext}', caption=cc1, message_thread_id=topic_thread_id)
                        os.remove(f'{name}.{ext}')
                    except FloodWait as e:
                        await m.reply_text(str(e))
                        time.sleep(e.x)
